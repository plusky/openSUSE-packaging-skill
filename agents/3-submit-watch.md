...
3. **Submit.** Pick the target by the rules in the reference:
   ...
   - **Brand-new package** you don't project-maintain → submit to its **devel project first** (`osc sr <home>/<pkg> <devel-project>`), then devel→Factory after acceptance. Can't create directly in a devel project you only package-maintain (403). Use `scripts/devel-of.sh` to check presence (404 = new). **If you hold the rights** (you maintain the devel project/package, unattended self-submission), accept the home→devel SR with **`osc request accept <id>`**, then file the Factory SR **explicitly** with `osc sr <devel-project> <pkg> openSUSE:Factory` (there is no `--forward` flag — osc's post-accept "forward?" prompt can't be answered non-interactively). When the user wants to maintain the new packages, add them to each `_meta` and mention the maintainer wish in the SR message.
