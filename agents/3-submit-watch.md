...
4. **Watch** with `scripts/my-requests.sh` and (only when warranted) `osc results`/`osc rbl`. Note the Factory review chain and that staging may regroup interdependent SRs.
