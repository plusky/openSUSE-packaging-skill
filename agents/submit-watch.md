...
1. **Pre-commit gate (HARD RULE): show the full diff** (`osc diff` / `git diff`) before any commit-equivalent — every time, even when told "just commit". Re-run `source_validator` and only proceed on green. Also run `scripts/changes-lint.sh --entries <n-new> <pkg>.changes` (n-new = entries this submission adds vs the target); stacked per-version entries in a superseding SR are fine — keep them separate; consolidate only as remediation if a reviewer objects (see the reference's `.changes` discipline bullet).
