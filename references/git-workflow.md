...
- **A Leap 16.x community-package update is a PR targeting the `leap-16.x` branch of `pool/<pkg>`** (Package Hub submission). Fork `pool/<pkg>`, branch off `leap-16.x`, sync it to the desired version, push to your fork, and open the PR with `base = leap-16.x`. The `openSUSE:Backports:SLE-16.0` OBS project is **scmsync read-only** (`<scmsync>…/products/PackageHub#leap-16.0`), fed from those branches — so `osc sr … openSUSE:Backports:SLE-16.0` is the *old* path and is wrong here. **`scripts/leap-sync.sh <pkg> [leap-branch]` automates the whole update flow** (clone → verify the leap branch exists & is behind factory → content-sync → LFS fetch → fork → push → open PR) — use it for any "push the latest Factory <pkg> to Leap" update; it refuses (and tells you why) for new-to-Leap packages.
- **Adding a *new* package to Leap is NOT a contributor self-service PR — it needs a `pool` maintainer.** A Gitea PR merges into an *existing* base branch, so you cannot create a `leap-16.x` branch by PR, and a normal contributor has **`push: false`** on `pool/<pkg>` (check `GET /api/v1/repos/pool/<pkg>` → `permissions.push`). New onboarding requires two maintainer/Package-Hub actions you can't do yourself: (1) **create the `leap-16.x` branch** in `pool/<pkg>` (content = factory), and (2) **register the package** as a submodule in the `products/PackageHub` repo's `leap-16.x` branch (`.gitmodules`: `path = <pkg>`, `url = ../../pool/<pkg>`, `branch = leap-16.x`). So onboarding a dependency cone (e.g. the FastMCP stack for `python-bugzilla-mcp`) is a **coordinated request to the Backports/Package Hub team**, not a stack of PRs. (Real wall: monero/monero-gui — no `leap-16.0` branch + `push:false` → couldn't onboard.)
- **To work out what a new cone still needs in Leap, check each dep across the three sources that feed Leap 16.0:** the SLE base **`SUSE:SLFO:1.2`** (`osc api /source/SUSE:SLFO:1.2/<pkg>/_meta`), **`openSUSE:Backports:SLE-16.0`**, and a **`leap-16.x` branch in `pool/<pkg>`**. A dep present in *any* of the three is already available; the gaps are what must be onboarded. (Real case: the FastMCP cone — ~24 deps already in SLFO/Backports, only `python-mcp/jsonref/griffelib/beartype/docstring-parser` missing, plus the 11 new stack packages.)
...
- **Review policy:** the Backports/Package Hub team approves; if the sources **match** openSUSE:Factory (or a maintained Leap) no extra sign-off is needed, but a deviation from Factory, or a submitter who isn't a regular Factory maintainer of the package, needs separate Factory-maintainer approval. So sync to *exactly* the Factory version when you can. (Real case: `pool/xmrig` leap-16.0 6.22.0→6.26.0 — PR `pool/xmrig#1` to the `leap-16.0` branch; you maintain xmrig in Factory so it qualified.)

### SLE-base packages: submit to the `slfo-x.y` branch, not onboard as new

A package that is **absent from `openSUSE:Backports:SLE-16.0` and has no `leap-16.x` branch**, yet ships in Leap 16.0, comes from the **SLE base `SUSE:SLFO:1.2`** (the shared base for SLES 16.0 *and* Leap 16.0). Check: `osc api /source/SUSE:SLFO:1.2/<pkg>/_meta` shows it `scmsync`'d from `pool/<pkg>?trackingbranch=slfo-1.2`. **You update it the same git way, just targeting the `slfo-1.2` branch** (and `slfo-main` for rolling SLFO) of `pool/<pkg>` — it is *not* a new-to-Leap onboarding. Content-sync `slfo-1.2` from `factory`, build, PR with `base = slfo-1.2`. **Caveat:** `slfo-1.2` feeds **SLES 16.0** (commercial), so the PR is reviewed by **SLFO/SUSE maintainers** — a higher bar than the community pools; sync to *exactly* the Factory version. (Real case: `wireless-regdb` was only in SLFO 1.2 @20250220 — PR `pool/wireless-regdb#1` to `slfo-1.2` synced it to Factory's 20260530.)

### Practical Factory→Leap update mechanics (obscpio / LFS packages)
...
- **Mirror 16.0 ⇄ 16.1 with a cherry-pick.** The `leap-16.0` and `leap-16.1` branches usually point at the same commit, so after committing the update on one, `git checkout -b leap-16.1-... origin/leap-16.1 && git cherry-pick <commit>` and push/PR the other — no second sync. (mbedtls, mbedtls-2, ofono, keepassxc, htop: one build, two PRs each.)
