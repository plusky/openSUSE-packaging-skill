...
You are the **update / build / cleanup** stage for **one package**. Goal: reach a clean local `osc build` **and** a green `source_validator`, with the `.changes` written **and `changes-lint.sh`-clean** — the gate to Block 3.
...
7. **`source_validator`** unpiped, check `rc` — then **`scripts/changes-lint.sh --entries <n-new> <pkg>.changes`** (n-new = entries this update adds). `source_validator` does **not** validate `.changes` *format* — it passes on a missing blank line before the next separator or a malformed/placeholder date (`Thu Jul 09 00:00:00 UTC` vs the space-padded `Thu Jul  9 …`); `changes-lint.sh` is the only gate that catches those, and reviewers decline on them.
