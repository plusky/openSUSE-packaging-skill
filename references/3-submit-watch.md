...
**Accept-and-forward (step 2→3 in one move, when you hold the rights).** If you *do* maintain the devel project (or the package within it) and are running the submission yourself, you don't need to manually file the second SR after the first is accepted — **`osc request accept <id> -m "…" --forward`** accepts the home→devel SR *and* automatically creates the follow-on submit request to the project the devel package is developed for (i.e. `openSUSE:Factory`). This is the "accept the request, then forward to Factory" path: one home→devel SR, accept it with `--forward`, and osc opens the devel→Factory SR for you (which then goes through the normal `factory-auto`/`licensedigger`/`opensuse-review-team` chain). Without rights, you stop after filing the home→devel SR and leave the accept to the devel-project maintainers — never try to self-accept where you lack permission (it'll 403). When the user has said they want to **maintain** the new packages, also add them to each package's `_meta` (`<person userid="<user>" role="maintainer"/>`, e.g. via `osc meta pkg <devel-project> <pkg> -e` or an API edit) and **say so in the SR message** so the devel-project reviewers expect the maintainer addition.
