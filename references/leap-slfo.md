...
**The flow:**
- `osc mbranch <pkg> openSUSE:Backports:SLE-15-SPx` (maintenance branch) often fails with *"no permission to modify project"* — that needs maintenance-incident rights. For local dev just use **`osc branch openSUSE:Backports:SLE-15-SPx <pkg>`**; it branches from the configured **`:Update`** project (checkout `home:<you>:branches:openSUSE:Backports:SLE-15-SPx:Update`).
- Local `osc build` aborts at an interactive trust prompt (the build root pulls from `SUSE:SLE-15-SPx:Update`) → build with **`--trust-all-projects`**.
- `osc sr` to `openSUSE:Backports:SLE-15-SPx:Update` prints `WARNING: … a maintenance incident request is being created` — that's **expected and correct** for a `:Update` target, not an error.
