...
Read `references/1-triage.md` (in the openSUSE-packaging skill) for the full method, then:
...
3. **Verify each candidate — this is the real work.** Per `references/1-triage.md`: compare by tag/commit **date, not version string** (renumbered/rolling tags, `v1.0` that's actually a 2014 downgrade); recognise **multi-track upstreams** (LTS lines, parallel sonames like `mbedtls-2`/`llvm15`) and **deliberately pinned** packages (read the latest `.changes`/spec comments for a pin rationale); and remember Repology lags the devel project (the devel spec may already be newer).

**Output contract:** a short report grouping candidates into **(a) likely-real updates** (with current→target and the upstream date), **(b) intentional pins / multi-track — skip** (with the reason), and **(c) Repology artifacts / downgrades — skip**. Do **not** edit specs or build here — hand the likely-real list back to the orchestrator, which drives Block 2 (`agents/2-update-build.md`) per package.
