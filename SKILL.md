...
Run `scripts/preflight.sh` first (HARD RULE — never repackage what devel already has), bump the version / run the source service, rebase or drop patches, run spec-cleaner, build locally with `osc build` (read the rpmlint summary, run `%check`), and fix FTBFS from the pitfalls catalog. **Gate to leave this block (and to gate any commit): a clean local `osc build`, a green `source_validator`, *and* a clean `scripts/changes-lint.sh --entries <n-new> <pkg>.changes`.** `source_validator` does **not** validate `.changes` *format* — it passes on a missing blank line before a separator or a malformed date (e.g. `Thu Jul 09 00:00:00 UTC` instead of the space-padded `Thu Jul  9 …`), both of which a human reviewer declines. `changes-lint.sh` is the only gate that catches them, so run it wherever a commit happens, not just before the SR.
