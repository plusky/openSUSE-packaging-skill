...
7. **Investigate the package's bugzilla bugs whenever you touch it — HARD RULE.** Any time you update, patch, clean, or rebuild a package, first run `mcp__bugzilla__bugs_quicksearch(query="<pkg>", status="NEW,ASSIGNED,REOPENED,IN_PROGRESS,CONFIRMED,NEEDINFO")` to find open bugs (CVE *and* functional) the change might fix or should reference. Cite the relevant `boo#NNNN` next to the fix in the `.changes`, and — with explicit user approval (bugzilla is read-only by default) — close what the change actually fixes. This is part of doing the package properly, not a later clean-up pass. See `references/bugzilla-cve-triage.md`.
8. **Survey other distributions whenever you touch a package — HARD RULE.** On any update or cleanup (not only when chasing a specific CVE), check how Fedora, Debian, Gentoo, Arch, Alpine and openEuler package it, to catch config/`./configure` options, patches, build fixes, or packaging improvements we're missing. See the distro list in `references/bugzilla-cve-triage.md` ("Surveying other distros for a fix/patch"). Cross-distro divergence also tells you whether the right fix is a downstream patch or a version/lineage bump.
