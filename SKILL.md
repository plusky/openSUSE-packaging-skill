...
- `distro-survey.sh <pkg> [factory-version]` — version (+ lineage hint) across Fedora/Debian/Gentoo/Arch/Alpine/openEuler/Void/NixOS/FreeBSD/OpenMandriva/Mageia in one call (the "survey other distros when you touch it" hard rule). Divergence flags a newer version, a different upstream lineage, or a patch worth pulling.
